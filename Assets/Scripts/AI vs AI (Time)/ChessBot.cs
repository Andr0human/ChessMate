...
public class ChessBot : MonoBehaviour {
    ...
    private int self_color, made_move_count = 0;
    ...
    public void CreateEntry(Vector2 alloted_time, int last_move) {
        string path = Application.streamingAssetsPath + "/chess_in.txt";

        string command;
        command = "-p ";
        if (last_move != 0)
            command += "-m " + last_move.ToString() + " ";
        command += "-tm " + alloted_time.x.ToString() + " " + alloted_time.y.ToString("0.##");
        command += " -e";
    ...
    private bool Get_Entry() {
        string path = Application.streamingAssetsPath + "/chess_out.txt";
    ...
    public void InitializeBot(string fen) {
        ...
        string in_path = Application.streamingAssetsPath + "/chess_in.txt";
        string out_path = Application.streamingAssetsPath + "/chess_out.txt";
        File.WriteAllText(in_path, "");
        File.WriteAllText(out_path, "");

        ProcessStartInfo startInfo = new ProcessStartInfo(Application.streamingAssetsPath + "/chessbot.exe") {
    ...
    public void MakeMove(ref ChessBoard _cb, int last_move) {
        ...
        Vector2 alloted_time = cs.Decide_Time(time_left, increment, ref _cb);
