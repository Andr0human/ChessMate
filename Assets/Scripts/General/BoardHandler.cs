...
public class BoardHandler : MonoBehaviour {
    ...
    public void Initialize_board(ref ChessBoard _cb) {
        white_tile = GameObject.Find("White Tile");
        black_tile = GameObject.Find("Black Tile");
        StartCoroutine(Board_Generator(_cb));
    }

    private IEnumerator Board_Generator(ChessBoard _cb) {
    ...
    private void SpawnPiece(int idx, int id) {
        Destroy(pieces[idx]);
        if (id == 0) return;
        if (id > 0) id--;
        id += 6;
    ...
    public void Board_reset(bool spare_last_move = false) {
        for (int i = 0; i < 64; i++) {
            tiles[i].GetComponent<TileSet>().reset_back(spare_last_move);
        }
    }

    public void Board_highLight(ulong end, ref ChessBoard cb) {
        while (end != 0) {
    ...
    public void Recreate(ref ChessBoard _cb) {
        for (int i = 0; i < 64; i++) {
            SpawnPiece(i, _cb.board[i]);
        }
    }

    public void MarkplayedMove(int move) {
